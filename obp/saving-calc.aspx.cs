...
public partial class obp_saving_calc : System.Web.UI.Page
{
    iClass c = new iClass();
    ...
    protected void btnCalculate_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!c.IsRecordExist("Select CustomrtID From CustomersData Where CustomerMobile='" + txtMobile.Text + "' AND delMark=0 AND CustomerActive=1 AND DeviceType='GOBP-Web'"))
            {
                errMsg = c.ErrNotification(2, "This customer data is not available, first add customer");
                return;
            }
            else
            {
                ...
                if (Session["medData"] != null)
                {
                    ...
                    else
                    {
                        ...
                        c.ExecuteQuery("Insert Into SavingCalc (CalcID, CalcDate, MobileNumber, FK_CustId, FK_AddressID, EnqStatus, DeviceType) " +
                            " Values (" + maxId + ", '" + DateTime.Now + "', '" + txtMobile.Text + "', " + customerId + ", 0, 0, 'GOBP-Enq')");
                    }

...
public partial class supportteam_followup_customer_report : System.Web.UI.Page
{
    iClass c = new iClass();
    ...
    private void FillGrid()
    {
        try
        {
            ...
            strQuery = @"SELECT 
                            MAX(a.[FK_OrderCustomerID]) as FK_OrderCustomerID,
                            ...
                            (SELECT COUNT([FlupID]) FROM [dbo].[FollowupOrders] WHERE [FK_CustomerId] = MAX(a.[FK_OrderCustomerID]) AND [FK_OrderId] = a.[OrderID]) as flCount,
                            ISNULL((SELECT TOP 1 c.[TeamPersonName] FROM [dbo].[SupportTeam] c INNER JOIN [dbo].[FollowupOrders] d ON c.[TeamID] = d.[FK_TeamMemberId] WHERE d.[FK_CustomerId] = MAX(a.[FK_OrderCustomerID]) AND d.[FK_OrderId] = a.[OrderID] ORDER BY d.[FlupID] DESC), 'NA') as flBy
                         ...
                         AND c.[OrdReAssign] = 0
                         AND a.[OrderDate] <= '" + DateTime.Now.Date
                         + "' AND a.[OrderDate] >= DATEADD(MONTH, -6, '" + DateTime.Now.Date + "')" 
                         + " AND DAY(a.OrderDate) = DATEPART(DAY, '" + DateTime.Now.Date + "')" 
                         + " AND a.[FK_OrderCustomerID] = " + Request.QueryString["custId"]
                         + " GROUP BY a.[OrderID]";

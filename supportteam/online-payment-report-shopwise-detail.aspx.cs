...
public partial class supportteam_online_payment_report_shopwise_detail : System.Web.UI.Page
{
    iClass c = new iClass();
    ...
    protected void gvOrder_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                litView.Text = "<a href=\"online-payment-report-shopwise-detail.aspx?id=" + e.Row.Cells[0].Text + "\" class=\"gView\" target=\"_blank\"></a>";
            }
        }
        catch (Exception ex)
        {
                    ...
                    foreach (DataRow row in dtPayment.Rows)
                    {
                        ...
                        if (firstSlabAmt > 0)
                        {
                            if (c.IsRecordExist("Select Payout_ID From FranchiseePayout Where FK_FranchID=" + fId +
                                ...
                                " AND Payout_Status=2 And Payout_Amount=" + firstSlabAmt + " AND (Payout_AccActivityDate IS NOT NULL OR Payout_AccActivityDate<>'')"))
                            {
                                object distId = c.GetReqData("FranchiseePayout", "FK_DistributorID", "FK_FranchID=" + fId + " AND ( CONVERT(varchar(20), Payout_FromDate, 112) = CONVERT(varchar(20), CAST('" + row["OrderDate"] + "' as datetime) , 112)) AND Payout_Status=2 And Payout_Amount=" + firstSlabAmt + " AND (Payout_AccActivityDate IS NOT NULL OR Payout_AccActivityDate<>'')");
                                ...
                                if (payStatus != DBNull.Value && payStatus != null && payStatus.ToString() != "")
                                {
                                    switch (Convert.ToInt32(payStatus))
                                    {
                                        case 1: status1 = "<i class=\"fas fa-check\"></i><span class=\"text-success text-bold\"> Approved By Account</span><span class=\"space1\"></span>" +
                                            ...
                                            actionButton1 = "<span class=\"btn btn-md btn-success\" onclick=\"ApprovePayment('" + date1 + "', '" + Request.QueryString["id"] + "', '" + firstSlabAmt.ToString("0.00") + "', '" + orderNumber + "');\">Approve</span>";
    ...
    public static int UpdatePayout(string dates, string franchId, string ordAmount, string orderNumber)
    {
        try
        {
                ...
                " AND Payout_Status=1 AND Payout_Order_Number='" + orderNumber + "' AND Payout_Amount=" + ordAmount + " AND (Payout_AccActivityDate IS NOT NULL OR Payout_AccActivityDate<>'')"))
            {
                ...
                {
                    c.ExecuteQuery("Update FranchiseePayout Set Payout_Status=3, Payout_PurchActivityDate='" + DateTime.Now + "' Where Payout_ID=" + maxId);
                }
            }
            else
            {

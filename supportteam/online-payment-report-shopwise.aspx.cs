...
public partial class supportteam_online_payment_report_shopwise : System.Web.UI.Page
{
    iClass c = new iClass();
        ...
        if (!IsPostBack)
        {
            if (Request.QueryString["id"] != null)
            {
                ...
                using (DataTable dtDate = c.GetDataTable("Select TOP 1 a.OrderDate From OrdersData a Inner Join OrdersAssign b " + 
                    ...
                    " Where b.OrdAssignStatus=7AND b.Fk_FranchID=" + Request.QueryString["id"] + 
    ...
    protected void gvOrder_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                litView.Text = "<a href=\"online-payment-report-shopwise.aspx?id=" + e.Row.Cells[0].Text + "\" class=\"gView\" target=\"_blank\"></a>";
            }
        }
        catch (Exception ex)
        {
    ...
    private void GetStats(int month, int year)
    {
        try
        {
            ...
            if (firstSlabAmt > 0)
            {
                ...
                " AND Payout_Status=2 AND (Payout_PurchActivityDate IS NOT NULL OR Payout_PurchActivityDate<>'')"))
                {
                ...
                else
                {
                    ...
                    if (payStatus != DBNull.Value && payStatus != null && payStatus.ToString() != "")
                    {
                        ...
                        {
                            //case 1: 
                            //    //status1 = "<span class=\"clrDelivered\">Approved By Account</span>"; 
                            //    status1 = "<span class=\"clrProcessing\">Pending From Purchase</span>";
                            //    string date1 = firstDayDate.ToString("dd/MM/yyyy") + " - " + firstDayDate.ToString("dd/MM/yyyy");
                            //    actionButton1 = "<span class=\"btn btn-md btn-success\" onclick=\"ApprovePayment('" + date1 + "', '" + Request.QueryString["id"] + "', '" + firstSlabAmt.ToString("0.00") + "');\">Mark as Paid</span>";
                            //    break;
                            //case 2: status1 = "<span class=\"clrDelivered\">Paid To Rajah</span>"; break;
                ...
                else
                {
                    ...
                    if (payStatus != DBNull.Value && payStatus != null && payStatus.ToString() != "")
                    {
                        switch (Convert.ToInt32(payStatus))
                        {
                            ...
                            //    status2 = "<span class=\"clrProcessing\">Pending From Purchase</span>";
                            //    string date2 = secondSlabFirstDate.ToString("dd/MM/yyyy") + " - " + secondSlabFirstDate.ToString("dd/MM/yyyy");
                            //    actionButton2 = "<span class=\"btn btn-md btn-success\" onclick=\"ApprovePayment('" + date2 + "', '" + Request.QueryString["id"] + "', '" + secondSlabAmt.ToString("0.00") + "');\">Mark as Paid</span>";
                            //    break;
                            //case 2: status2 = "<span class=\"clrDelivered\">Paid To Rajah</span>"; break;
                ...
                else
                {
                    ...
                    if (payStatus != DBNull.Value && payStatus != null && payStatus.ToString() != "")
                    {
                        switch (Convert.ToInt32(payStatus))
                        {
                            ...
                            //    status3 = "<span class=\"clrProcessing\">Pending From Purchase</span>";
                            //    string date3 = thirdSlabFirstDate.ToString("dd/MM/yyyy") + " - " + thirdSlab.ToString("dd/MM/yyyy");
                            //    actionButton3 = "<span class=\"btn btn-md btn-success\" onclick=\"ApprovePayment('" + date3 + "', '" + Request.QueryString["id"] + "', '" + thirdSlabAmt.ToString("0.00") + "');\">Mark as Paid</span>";
                            //    break;
                            //case 2: status3 = "<span class=\"clrDelivered\">Paid To Rajah</span>"; break;
    ...
    public static int UpdatePayout(string dates, string franchId, string ordAmount)
    {
        try
        {
            ...
            if (c.IsRecordExist("Select Payout_ID From FranchiseePayout Where FK_FranchID=" + franchId +
                ...
                " AND Payout_Status=1 AND Payout_Amount=" + ordAmount + " AND (Payout_AccActivityDate IS NOT NULL OR Payout_AccActivityDate<>'')"))
            {
                maxId = Convert.ToInt32(c.GetReqData("FranchiseePayout", "Payout_ID", "FK_FranchID=" + franchId + " AND ( CONVERT(varchar(20), Payout_FromDate, 112) >= CONVERT(varchar(20), CAST('" + fDate + "' as datetime) , 112) AND CONVERT(varchar(20), Payout_ToDate, 112) <= CONVERT(varchar(20), CAST('" + tDate + "' as datetime), 112 ) ) AND Payout_Status=1 AND Payout_Amount=" + ordAmount + " AND (Payout_AccActivityDate IS NOT NULL OR Payout_AccActivityDate<>'')"));

                c.ExecuteQuery("Update FranchiseePayout Set Payout_Status=2, Payout_PurchActivityDate='" + DateTime.Now + "' Where Payout_ID=" + maxId);
            }
            else
            {
                return 4;

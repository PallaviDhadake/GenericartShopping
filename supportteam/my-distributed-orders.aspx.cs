...
public partial class supportteam_my_distributed_orders : System.Web.UI.Page
{
    iClass c = new iClass();
                ...
                if (Request.QueryString["shop"] != null)
                {
                    if (Request.QueryString["shop"] == "own")
                    {
                        strQuery = @"SELECT "
                                        ...
                                        + "(SELECT COUNT([FlupID]) FROM [dbo].[FollowupOrders] WHERE [FK_CustomerId] = a.[FK_OrderCustomerID] AND [FK_OrderId] = f.[FK_OrderId]) as flCount,"
                                        + "ISNULL((SELECT TOP 1 c.[TeamPersonName] FROM [dbo].[SupportTeam] c INNER JOIN [dbo].[FollowupOrders] d ON c.[TeamID] = d.[FK_TeamMemberId] WHERE d.[FK_CustomerId] = a.[FK_OrderCustomerID] AND d.[FK_OrderId] = f.[FK_OrderId] ORDER BY d.[FlupID] DESC), 'NA') as flBy"
                                        + " FROM [dbo].[OrdersData] a"
                                        + " INNER JOIN [dbo].[FollowupAssign] f ON a.[OrderID] = f.[FK_OrderId]"

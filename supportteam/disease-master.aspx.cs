...
public partial class supportteam_disease_master : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            pgTitle = Request.QueryString["action"] != null ? Request.QueryString["action"] == "new" ? "Add New Concern" : "Edit Concern" : "Concern Master";
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["action"] != null)
                {
                    ...
                    else
                    {
                        ...
                        GetDisease(Convert.ToInt32(Request.QueryString["id"]));
    ...
    private void FillGrid()
    {
        try
        {
            using (DataTable dtDisease = c.GetDataTable("Select DiseaseId, DiseaseName From DiseaseData Where delMark=0 Order By DiseaseId DESC"))
            {
                gvDisease.DataSource = dtDisease;
                gvDisease.DataBind();
                if (gvDisease.Rows.Count > 0)
                {
                    gvDisease.UseAccessibleHeader = true;
                    gvDisease.HeaderRow.TableSection = TableRowSection.TableHeader;
                }
    ...
    protected void gvDisease_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                litAnch.Text = "<a href=\"disease-master.aspx?action=edit&id=" + e.Row.Cells[0].Text + "\" class=\"gAnch\"></a>";
            }
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('error', 'Error Occoured While Processing');", true);
            c.ErrorLogHandler(this.ToString(), "gvDisease_RowDataBound", ex.Message.ToString());
            return;
        }
    }
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            int maxId = Request.QueryString["id"] != null ? Convert.ToInt32(Request.QueryString["id"]) : c.NextId("DiseaseData", "DiseaseId");
            ...
            if (lblId.Text == "[New]")
            {
                ...
                c.ExecuteQuery("Insert Into DiseaseData (DiseaseId, DiseaseName, DiseaseCoverPhoto, delMark) Values(" + maxId +
                    ", '" + txtName.Text + "', '" + imgName + "', 0)");

                ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('success', 'Concern added successfully');", true);
            }
            else
            {
                c.ExecuteQuery("Update DiseaseData Set DiseaseName='" + txtName.Text +
                    "' Where DiseaseId=" + maxId);

                if (fuImg.HasFile)
                {
                    c.ExecuteQuery("Update DiseaseData Set DiseaseCoverPhoto='" + imgName + "' Where DiseaseId=" + maxId);
                }

                ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('success', 'Concern updated successfuly');", true);
            }

            txtName.Text ="";
            Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "waitAndMove('disease-master.aspx', 2000);", true);
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('error', 'Error Occoured While Processing');", true);
    ...
    private void GetDisease(int disIdX)
    {
        try
        {
            using (DataTable dtTest = c.GetDataTable("Select * From DiseaseData Where DiseaseId=" + disIdX))
            {
                if (dtTest.Rows.Count > 0)
                {
                    DataRow row = dtTest.Rows[0];

                    lblId.Text = disIdX.ToString();

                    txtName.Text = row["DiseaseName"].ToString();
                   

                    if (row["DiseaseCoverPhoto"] != DBNull.Value && row["DiseaseCoverPhoto"] != null && row["DiseaseCoverPhoto"].ToString() != "" && row["DiseaseCoverPhoto"].ToString() != "no-photo.png")
                    {
                        disImg = "<img src=\"" + Master.rootPath + "upload/diseases/" + row["DiseaseCoverPhoto"].ToString() + "\" width=\"200\" />";
                    }
                }
                else
                {
        ...
        {
            ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('error', 'Error Occoured While Processing');", true);
            c.ErrorLogHandler(this.ToString(), "GetDisease", ex.Message.ToString());
            return;
        }
    }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            c.ExecuteQuery("Update DiseaseData Set delMark=1 Where DiseaseId=" + Convert.ToInt32(lblId.Text));
            ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('success', 'Concern info deleted');", true);
            Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "waitAndMove('disease-master.aspx', 2000);", true);
        }
        catch (Exception ex)
        {
    ...
    {
        Response.Redirect("disease-master.aspx");
    }
    ...
    private void ImageUploadProcess(string imgName)
    {
        try
        {
            string origImgPath = "~/upload/diseases/original";
            string normalImgPath = "~/upload/diseases/";
            string thumbRawImgPath = "~/upload/diseases/thumbRaw/";
        ...
        try
        {
                ...
                " AND a.FK_SubCategoryID=" + ddrSubCategory.SelectedValue + " Order By a.ProductID DESC")) 
            {
                ...
                foreach (GridViewRow gRow in gvProducts.Rows)
                {
                    ...
                    if (c.IsRecordExist("Select DisProID From DiseaseProducts Where FK_DiseaseID=" + Request.QueryString["id"] + " And FK_ProductID=" + gRow.Cells[0].Text))
                        chk.Checked = true;
                    else
                        chk.Checked = false;
                }
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                int maxId = c.NextId("DiseaseProducts", "DisProID");

                if (!c.IsRecordExist("Select DisProID From DiseaseProducts Where FK_ProductID=" + prId + " AND FK_DiseaseID=" + Request.QueryString["id"]))
                {
                    c.ExecuteQuery("Insert Into DiseaseProducts (DisProID, FK_ProductID, FK_DiseaseID) Values (" +
                        maxId + ", " + prId + ", " + Request.QueryString["id"] + ")");
                }
            }

            ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('success', 'Products added to concern');", true);
            Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "waitAndMove('disease-master.aspx?action=edit&id=" + Request.QueryString["id"] + "', 2000);", true);
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('error', 'Error Occoured While Processing');", true);
    ...
    protected void gvProducts_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (c.IsRecordExist("Select DisProID From DiseaseProducts Where FK_ProductID=" + e.Row.Cells[0].Text + " AND FK_DiseaseID=" + Request.QueryString["id"]))
                {
                    btnDel.Visible = true;
                    chk.Visible = false;
                }
    ...
    protected void gvProducts_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            {
                c.ExecuteQuery("Delete From DiseaseProducts Where FK_ProductID=" + gRow.Cells[0].Text + " AND FK_DiseaseID=" + Request.QueryString["id"]);
            }

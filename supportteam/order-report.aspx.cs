...
public partial class supportteam_order_report : System.Web.UI.Page
{
    iClass c = new iClass();
                    ...
                    case "new":
                        strQuery = @"SELECT DISTINCT 
                                        ...
                                        ISNULL(a.[DeviceType], '-') AS DeviceType,
                                     	(SELECT COUNT([FK_DetailProductID]) FROM[dbo].[OrdersDetails] WHERE[FK_DetailOrderID] = a.[OrderID]) AS ProductCount,
                                        LEFT (STUFF((SELECT ', ' + RTRIM(LTRIM([ProductName])) FROM[dbo].[ProductsData] WHERE[ProductID] IN(SELECT[FK_DetailProductID] FROM[dbo].[OrdersDetails] WHERE[FK_DetailOrderID] = a.[OrderID]) FOR XML PATH('')), 1, 1, '' ) , 200) AS CartProducts,
                                        a.[OrderStatus]
                                     FROM[dbo].[OrdersData] a
                                     INNER JOIN[dbo].[CustomersData] b ON a.[FK_OrderCustomerID] = b.[CustomrtID]
                                     WHERE a.[OrderStatus] = 1
                                     AND a.[OrderType] = 1
                                     AND(b.[CustomerFavShop] IS NULL OR b.[CustomerFavShop] = 0 OR b.[CustomerFavShop] IS NOT NULL OR b.[CustomerFavShop] <> 0)
                                     ORDER BY ordDate DESC, a.[OrderID] DESC";
                        break;
                }
            }
            else

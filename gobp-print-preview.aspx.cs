...
public partial class gobp_print_preview : System.Web.UI.Page
{
    iClass c = new iClass();
    public string pgTitle, enqCount, headInfo, clsName, rootPath, errMsg;
    public string editAnch, pgHeadCaption;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                {
                    pgTitle = "GOBP Registration Details";
                    GetGOBPEnqData(Convert.ToInt32(Request.QueryString["id"]));
                    //headInfo = c.GetHeadInfo(3, Convert.ToInt32(Request.QueryString["id"]));
                }
                else
                {
    ...
    private void GetGOBPEnqData(int enqIdX)
    {
        try
        {
            using (DataTable dtEnq = c.GetDataTable("SELECT * FROM [dbo].[OBPData] WHERE [OBP_ID] = " + enqIdX))
            {
                ...
                {                    
                    ...
                    enqData[3] = row["OBP_ApplicantName"] != DBNull.Value ? row["OBP_ApplicantName"].ToString() : "";
                    ...
                    enqData[9] = c.GetReqData("[dbo].[StatesData]", "[StateName]", "[StateID] = " + row["OBP_StateID"]).ToString();
                    enqData[10] = c.GetReqData("[dbo].[DistrictsData]", "[DistrictName]", "[DistrictId] = " + row["OBP_DistrictID"]).ToString();
                    ...
                    enqData[0] = c.GetReqData("[dbo].[OBPTypes]", "[OBPTypeName]", "[OBPTypeID] = " + row["OBP_FKTypeID"]).ToString();
                    ...
                    if (row["OBP_IDProof1"] != DBNull.Value && row["OBP_IDProof1"].ToString() != "" && row["OBP_IDProof1"].ToString() != null)
                    {
                        ...
                        //enqData[28] = "<a href=\"../upload/idproof/" + row["idProof"].ToString() + "\" target=\"_blank\" title=\"View Larger\" ><img src=\"../upload/idproof/" + row["idProof"].ToString() + "\" style=\"width:150px;\" /></a>";

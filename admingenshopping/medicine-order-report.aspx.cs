...
public partial class admingenshopping_medicine_order_report : System.Web.UI.Page
{
    iClass c = new iClass();
    ...
    private void FillGrid()
    {
        try
        {
            ...
            if (txtFDate.Text != "" && txtToDate.Text != "")
            {
                ...
                string dateCondition = "( (CONVERT(varchar(20), OrderDate, 112) >= CONVERT(varchar(20), CAST('" + fromDate + "' as DATETIME), 112) ) AND "
                    + " (CONVERT(varchar(20), OrderDate, 112) <= CONVERT(varchar(20), CAST('" + toDate + "' as DATETIME), 112)))";

                string ordIds = GetOrderIds(dateCondition);
                if (ordIds != "0")
                {
                    strQuery = "Select Distinct a.FK_DetailProductID, a.OrdDetailSKU, b.ProductName, " +
                        " (Select Count(FK_DetailOrderID) From OrdersDetails Where FK_DetailProductID=a.FK_DetailProductID) as totalOrders, " +
                        " (Select Sum(OrdDetailQTY) From OrdersDetails Where FK_DetailProductID=a.FK_DetailProductID) as totalQty, " +
    ...
    private string GetOrderIds(string whereCon)
    {
        try
        {
            using (DataTable dtOrd = c.GetDataTable("Select OrderID From OrdersData Where " + whereCon))
            {
                string ids = "";
                if (dtOrd.Rows.Count > 0)
                {
                    ...
                    {
                        if (ids == "")
                            ids = row["OrderID"].ToString();
                        else
                            ids = ids + "," + row["OrderID"].ToString();
                    }

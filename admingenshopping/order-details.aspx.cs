...
public partial class admingenshopping_order_details : System.Web.UI.Page
{
    iClass c = new iClass();
            ...
            if (Request.QueryString["id"] != null)
            {
                ...
                if (Request.QueryString["type"] != null && Request.QueryString["prId"] != null)
                {
                    ...
                    if (Request.QueryString["ref"] != null)
                    {
                        if (Request.QueryString["ref"] == "rx")
                        {
                            redirect = "order-details.aspx?id=" + Request.QueryString["id"] + "&ref=rx";
                        }
                    }
                    else
                    {
                        redirect = "order-details.aspx?id=" + Request.QueryString["id"];
                    }
    ...
    //        using (DataTable dtOrd = c.GetDataTable("select FK_DetailProductID from OrderDetails where FK_DetailOrderID = "+ Request.QueryString["id"] ))
    //        {
    //            if (dtOrd.Rows.Count > 0)
    //            {
    //                foreach (DataRow row in dtOrd.Rows)
    //                {
    //                    if (row["FK_DetailProductID"] != DBNull.Value && row["FK_DetailProductID"] != null && row["FK_DetailProductID"].ToString() != "")
    //                    {
    //                        if (prIds != "")
    //                            prIds = prIds + "," + row["FK_DetailProductID"].ToString();
    //                        else
    //                            prIds = row["FK_DetailProductID"].ToString();
    //                    }
    ...
    //            if (prIds != "0")
    //            {
    //                using (DataTable dtProduct = c.GetDataTable("select  from ProductsData where FK_DetailProductID In (" + prIds + ")"))
    //                {
    ...
    public void GetOrdersData(int Idx)
    {
        try
        {
            ...
            //using (DataTable dtCust = c.GetDataTable("Select * From CustomersData Where CustomrtID = " + customerId))
            //{
            //    if (dtCust.Rows.Count > 0)
            //    {
            //        DataRow bRow = dtCust.Rows[0];
            //        ordCustData[0] = bRow["CustomerName"].ToString();
            //        ordCustData[1] = bRow["CustomerMobile"].ToString();
            //        ordCustData[2] = bRow["CustomerEmail"].ToString();
            //        ordCustData[3] = bRow["CustomerCity"].ToString();
            //        ordCustData[4] = bRow["CustomerState"].ToString();
            //        ordCustData[5] = bRow["CustomerPincode"].ToString();
            //        ordCustData[6] = bRow["CustomerAddress"].ToString();
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //{
            //    ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('warning', 'Please select Order Status');", true);
            //    GetOrdersData(Convert.ToInt32(Request.QueryString["id"]));
            //    return;
            //}
            //else
            //{
            //    c.ExecuteQuery("Update OrdersData Set OrderStatus=" + orderStatus.SelectedValue + "Where OrderID=" + Convert.ToInt32(Request.QueryString["id"]));
            //    GetOrdersData(Convert.ToInt32(Request.QueryString["id"]));
            //    ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('success', 'Status Changed successfully');", true);
            //    Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "waitAndMove('order-details.aspx', 2000);", true);
            //}
        }
        catch (Exception ex)
        {

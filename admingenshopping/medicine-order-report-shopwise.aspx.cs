...
public partial class admingenshopping_medicine_order_report_shopwise : System.Web.UI.Page
{
    iClass c = new iClass();
                    ...
                    if (Convert.ToInt32(ddrZh.SelectedValue) > 0)
                    {
                        if (Convert.ToInt32(ddrDh.SelectedValue) > 0)
                        {
                            strQuery = "Select Distinct a.Fk_FranchID, b.FranchName, b.FranchShopCode, " +
                                ...
                                " (Select Count(FK_OrderID) From OrdersAssign Where Fk_FranchID=a.Fk_FranchID AND OrdAssignStatus=0 AND OrdReAssign=0 AND " + dateCondition + ") as pendingOrd, " +
                                " (Select Count(FK_OrderID) From OrdersAssign Where Fk_FranchID=a.Fk_FranchID AND OrdAssignStatus=5 AND OrdReAssign=0 AND " + dateCondition + ") as inprocOrd, " +
                                " (Select Count(FK_OrderID) From OrdersAssign Where Fk_FranchID=a.Fk_FranchID AND OrdAssignStatus=6 AND OrdReAssign=0 AND " + dateCondition + ") as shippedOrd, " +
                                " (Select Count(FK_OrderID) From OrdersAssign Where Fk_FranchID=a.Fk_FranchID AND OrdAssignStatus=7 AND OrdReAssign=0 AND " + dateCondition + ") as deliveredOrd, " +
                                " (Select Count(FK_OrderID) From OrdersAssign Where Fk_FranchID=a.Fk_FranchID AND OrdAssignStatus=2 AND (OrdReAssign=1 OR OrdReAssign=0) AND " + dateCondition + ") as rejectedOrd " +
    ...
    protected void gvOrder_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //string ordIds = GetOrderIds("Select Distinct FK_OrderID From OrdersAssign Where Fk_FranchID=a.Fk_FranchID AND OrdAssignStatus=7 AND OrdReAssign=0) AND " + dateCondition);
                //e.Row.Cells[6].Text = c.returnAggregate("Select SUM(OrdDetailAmount) From OrdersDetails Where FK_DetailOrderID IN (" + ordIds + ")").ToString();

                //string totordIds = GetOrderIds("Select Distinct FK_OrderID From OrdersAssign Where Fk_FranchID=a.Fk_FranchID AND OrdAssignStatus<>2 AND OrdReAssign=0) AND " + dateCondition);
                //e.Row.Cells[4].Text = c.returnAggregate("Select SUM(OrdDetailAmount) From OrdersDetails Where FK_DetailOrderID IN (" + totordIds + ")").ToString();

                //string rejordIds = GetOrderIds("Select Distinct FK_OrderID From OrdersAssign Where Fk_FranchID=a.Fk_FranchID AND OrdAssignStatus=2 AND (OrdReAssign=1 OR OrdReAssign=0)) AND " + dateCondition);
                //e.Row.Cells[11].Text = c.returnAggregate("Select SUM(OrdDetailAmount) From OrdersDetails Where FK_DetailOrderID IN (" + rejordIds + ")").ToString();
            }
        }
        catch (Exception ex)
        {
    ...
    private string GetOrderIds(string strQuery)
    {
        try
        {
            string ordIds = "";
            using (DataTable dtOrd = c.GetDataTable(strQuery))
            {
                if (dtOrd.Rows.Count > 0)
                {
                    ...
                    {
                        if (ordIds == "")
                            ordIds = row["FK_OrderID"].ToString();
                        else
                            ordIds = ordIds + "," + row["FK_OrderID"].ToString();
                    }

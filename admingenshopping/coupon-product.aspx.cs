...
public partial class admingenshopping_coupon_product : System.Web.UI.Page
{
    iClass c = new iClass();
        ...
        try
        {
            using (DataTable dtNws = c.GetDataTable("Select * From coupon Where coupon_id=" + nIdX))
            {
                if (dtNws.Rows.Count > 0)
                {
                    ...
                    txtPurchaseQty.Text = row["coupon_product_qty"].ToString();
                    txtProductOffer.Text = c.GetReqData("ProductsData", "ProductName", "ProductID=" + Convert.ToInt32(row["coupon_product_id"].ToString())).ToString();
    ...
    private void FillGrid()
    {
        try
        {
            using (DataTable dtBlog = c.GetDataTable("Select coupon_id, Convert(varchar(20), coupon_effective_date, 103) as cpnDate, coupon_code, coupon_head From coupon Where coupon_type='product' Order By coupon_id DESC"))
            {
    ...
    protected void gvCoupon_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                litAnch.Text = "<a href=\"coupon-product.aspx?action=edit&id=" + e.Row.Cells[0].Text + "\" class=\"gAnch\" title=\"View/Edit\"></a>";
            }
        }
        catch (Exception ex)
        {
    ...
    {
        Response.Redirect("coupon-product.aspx", false);
    }
    ...
    public static string SaveCouponData(coupon cinfo)
    {
        ...
        // Get Offer Product's ID
        refIdInfo = c.GetReqData("ProductsData", "ProductID", "ProductName='" + cinfo.CouponProductOffer + "'");
        cinfo.CouponProductId = Convert.ToInt32(refIdInfo);

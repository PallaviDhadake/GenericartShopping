...
public partial class obpmanager_overall_dashboard : System.Web.UI.Page
{
    iClass c = new iClass();
    ...
    private void GetCount()
    {
        try
        {
            arrCounts[0] = c.returnAggregate(@"SELECT COUNT([OBP_ID]) 
                                               FROM [dbo].[OBPData] WHERE [OBP_DelMark] = 0").ToString();
            ...
            arrCounts[1] = c.returnAggregate(@"SELECT COUNT(DISTINCT OP.[OBP_ID])
                                               ...
                                               AND CD.[FK_ObpID] > 0 AND OP.[OBP_DelMark] = 0").ToString();

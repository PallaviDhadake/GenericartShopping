...
public partial class bdm_order_details : System.Web.UI.Page
{
    iClass c = new iClass();
    public string customerId, errMsg, orderCount, shippingCharges, prescriptionStr, paymentslip, rdrUrl, mreq, assignDetails, gobpName;
    ...
    public void GetOrdersData(int Idx)
    {
        try
        {
            ...
            if (c.IsRecordExist("Select OrderID From OrdersData Where OrderID=" + Idx))
            {
                ...
                using (DataTable dtRx = c.GetDataTable("Select OrdPayId, OrdPaySlip From OrdersPaymentSlip Where FK_OrderId = " + Idx))
                {
                    if (dtRx.Rows.Count > 0)
                    {
                        int bCount = 0;

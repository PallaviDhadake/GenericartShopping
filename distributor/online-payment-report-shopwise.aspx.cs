...
public partial class distributor_online_payment_report_shopwise : System.Web.UI.Page
{
    iClass c = new iClass();
    ...
    private void FillGrid()
    {
        try
        {
            ...
            using (DataTable dtPay = c.GetDataTable("Select Distinct a.FranchID, a.FranchName, a.FranchShopCode, " +
                    ...
                    " Where oa.Fk_FranchID=a.FranchID AND oa.OrdAssignStatus=7 AND oa.OrdReAssign=0 AND (od.OrderPaymentTxnId IS NOT NULL AND od.OrderPaymentTxnId<>'') AND (opl.OPL_transtatus='SUCCESS' OR opl.OPL_transtatus='paid') ), 0) AS float) - " +
    ...
    private void GetStats(int month, int year)
    {
        try
        {
            ...
            if (firstSlabAmt > 0)
            {
                ...
                " AND Payout_Status=3 AND (Payout_RajahActivityDate IS NOT NULL OR Payout_RajahActivityDate<>'')"))
                {
                    status1 = "<i class=\"fas fa-check\"></i><span class=\"text-success text-bold\"> Approved By Account</span> <span class=\"space1\"></span>" +
                                ...
                                "<i class=\"fas fa-check\"></i><span class=\"text-success text-bold\"> Received & Updated By Rajah</span>";
                }
                else
                {
                    //if (c.IsRecordExist("Select Payout_ID From FranchiseePayout Where FK_FranchID=" + fId +
                    ...
                    //{
                    //    //status1 = "<span class=\"clrDelivered\">Paid By Purchase</span>";

                    //    status1 = "<span class=\"clrProcessing\">Pending From Purchase</span>";
                    ...
                    if (payStatus != DBNull.Value && payStatus != null && payStatus.ToString() != "")
                    {
                        ...
                        {
                            //case 1: status1 = "<span class=\"clrDelivered\">Approved By Account</span>"; break;
                            //case 2: 
                            //    status1 = "<span class=\"clrProcessing\">Pending From Rajah</span>";
                            //    string date1 = firstDayDate.ToString("dd/MM/yyyy") + " - " + firstDayDate.ToString("dd/MM/yyyy");
                            //    actionButton1 = "<span class=\"btn btn-md btn-success\" onclick=\"ApprovePayment('" + date1 + "', '" + Request.QueryString["id"] + "', '" + firstSlabAmt.ToString("0.00") + "');\">Mark as Received & Update</span>";
                            //    break;
                            //case 3: status1 = "<span class=\"clrDelivered\">Received & Updated By Rajah</span>"; break;
            ...
            if (secondSlabAmt > 0)
            {
                ...
                else
                {
                    ...
                    //" AND Payout_Status=1 AND (Payout_AccActivityDate IS NOT NULL OR Payout_AccActivityDate<>'')"))
                    //{
                    //    //status2 = "<span class=\"clrDelivered\">Paid By Purchase</span>";

                    //    status2 = "<span class=\"clrProcessing\">Pending From Purchase</span>";
                    ...
                    if (payStatus != DBNull.Value && payStatus != null && payStatus.ToString() != "")
                    {
                        switch (Convert.ToInt32(payStatus))
                        {
                            //case 1: status2 = "<span class=\"clrDelivered\">Approved By Account</span>"; break;
                            //case 2: 
                            //    status2 = "<span class=\"clrProcessing\">Pending From Rajah</span>";
                            //    string date2 = secondSlabFirstDate.ToString("dd/MM/yyyy") + " - " + secondSlabFirstDate.ToString("dd/MM/yyyy");
                            //    actionButton2 = "<span class=\"btn btn-md btn-success\" onclick=\"ApprovePayment('" + date2 + "', '" + Request.QueryString["id"] + "', '" + secondSlabAmt.ToString("0.00") + "');\">Mark as Received & Update</span>";
                            //    break;
                            //case 3: status2 = "<span class=\"clrDelivered\">Received & Updated By Rajah</span>"; break;
            ...
            if (thirdSlabAmt > 0)
            {
                ...
                else
                {
                    ...
                    //" AND Payout_Status=1 AND (Payout_AccActivityDate IS NOT NULL OR Payout_AccActivityDate<>'')"))
                    //{
                    //    //status3 = "<span class=\"clrDelivered\">Approved By Account</span>";
                    //    status3 = "<span class=\"clrProcessing\">Pending From Purchase</span>";
                    ...
                    if (payStatus != DBNull.Value && payStatus != null && payStatus.ToString() != "")
                    {
                        switch (Convert.ToInt32(payStatus))
                        {
                            //case 1: status3 = "<span class=\"clrDelivered\">Approved By Account</span>"; break;
                            //case 2: 
                            //    status3 = "<span class=\"clrProcessing\">Pending From Rajah</span>";
                            //    string date3 = thirdSlabFirstDate.ToString("dd/MM/yyyy") + " - " + thirdSlab.ToString("dd/MM/yyyy");
                            //    actionButton3 = "<span class=\"btn btn-md btn-success\" onclick=\"ApprovePayment('" + date3 + "', '" + Request.QueryString["id"] + "', '" + thirdSlabAmt.ToString("0.00") + "');\">Mark as Received & Update</span>";
                            //    break;
                            //case 3: status3 = "<span class=\"clrDelivered\">Received & Updated By Rajah</span>"; break;
    ...
    public static int UpdatePayout(string dates, string franchId, string ordAmount)
    {
        try
        {
            ...
            if (c.IsRecordExist("Select Payout_ID From FranchiseePayout Where FK_FranchID=" + franchId +
                    " AND ( CONVERT(varchar(20), Payout_FromDate, 112) >= CONVERT(varchar(20), CAST('" + fDate + "' as datetime) , 112) " +
                " AND CONVERT(varchar(20), Payout_ToDate, 112) <= CONVERT(varchar(20), CAST('" + tDate + "' as datetime), 112 ) ) " +
                " AND Payout_Status=2 AND Payout_Amount=" + ordAmount + " AND (Payout_PurchActivityDate IS NOT NULL OR Payout_PurchActivityDate<>'')"))
            {
                maxId = Convert.ToInt32(c.GetReqData("FranchiseePayout", "Payout_ID", "FK_FranchID=" + franchId + " AND ( CONVERT(varchar(20), Payout_FromDate, 112) >= CONVERT(varchar(20), CAST('" + fDate + "' as datetime) , 112) AND CONVERT(varchar(20), Payout_ToDate, 112) <= CONVERT(varchar(20), CAST('" + tDate + "' as datetime), 112 ) ) AND Payout_Status=2 AND Payout_Amount=" + ordAmount + " AND (Payout_PurchActivityDate IS NOT NULL OR Payout_PurchActivityDate<>'')"));

                c.ExecuteQuery("Update FranchiseePayout Set Payout_Status=3, Payout_RajahActivityDate='" + DateTime.Now + "' Where Payout_ID=" + maxId);
            }
            else
            {
                return 4;

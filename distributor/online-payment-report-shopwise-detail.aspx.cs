...
public partial class distributor_online_payment_report_shopwise_detail : System.Web.UI.Page
{
    iClass c = new iClass();
                    ...
                    foreach (DataRow row in dtPayment.Rows)
                    {
                        ...
                        if (firstSlabAmt > 0)
                        {
                            if (c.IsRecordExist("Select Payout_ID From FranchiseePayout Where FK_FranchID=" + fId +
                                ...
                                " AND Payout_Status=3 And Payout_Amount=" + firstSlabAmt + " AND (Payout_AccActivityDate IS NOT NULL OR Payout_AccActivityDate<>'')"))
                            {

                                object distId = c.GetReqData("FranchiseePayout", "FK_DistributorID", "FK_FranchID=" + fId + " AND ( CONVERT(varchar(20), Payout_FromDate, 112) = CONVERT(varchar(20), CAST('" + row["OrderDate"] + "' as datetime) , 112)) AND Payout_Status=3 And Payout_Amount=" + firstSlabAmt + " AND (Payout_AccActivityDate IS NOT NULL OR Payout_AccActivityDate<>'')");
                               ...
                               "<i class=\"fas fa-check\"></i><span class=\"text-success text-bold\"> Received & Updated By " + strRajah + "</span>";
                            }
                            else
                            {
    ...
    public static int UpdatePayout(string dates, string franchId, string ordAmount, string orderNumber)
    {
        try
        {
            ...
            if (c.IsRecordExist("Select Payout_ID From FranchiseePayout Where FK_FranchID=" + franchId +
                ...
                " AND Payout_Status=2 AND Payout_Order_Number='" + orderNumber + "' AND Payout_Amount=" + ordAmount + " AND (Payout_PurchActivityDate IS NOT NULL OR Payout_PurchActivityDate<>'')"))
            {
                maxId = Convert.ToInt32(c.GetReqData("FranchiseePayout", "Payout_ID", "FK_FranchID=" + franchId + " AND ( CONVERT(varchar(20), Payout_FromDate, 112) >= CONVERT(varchar(20), CAST('" + fDate + "' as datetime) , 112) AND CONVERT(varchar(20), Payout_ToDate, 112) <= CONVERT(varchar(20), CAST('" + tDate + "' as datetime), 112 ) ) AND Payout_Status=2 AND Payout_Order_Number='" + orderNumber + "' AND Payout_Amount=" + ordAmount + " AND (Payout_PurchActivityDate IS NOT NULL OR Payout_PurchActivityDate<>'')"));

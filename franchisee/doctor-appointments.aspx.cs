...
public partial class franchisee_doctor_appointments : System.Web.UI.Page
{
    iClass c = new iClass();
    public string errMsg, deviceType, frCode, appUrl;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            frCode = c.GetReqData("FranchiseeData", "FranchShopCode", "FranchID=" + Session["adminFranchisee"]).ToString();
            appUrl = Master.rootPath + "book-appointment/dr-shruti-salgar-5?code=" + frCode;
        }
        catch (Exception ex)
        {
    ...
    private void FillGrid()
    {
        try
        {
            ...
            strQuery = "Select DocAppID, Convert(varchar(20), DocAppDate, 103) as appDate, isnull(DeviceType, '-') as DeviceType, " +
                ...
                " DocAppName, DocAppMobile, DocAppAge, DocAppStatus, isnull(FK_DocID, 0) as FK_DocID From DoctorsAppointmentData Where DocRefShopCode='" + shopCode + "'";

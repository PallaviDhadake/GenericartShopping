...
public partial class franchisee_order_details : System.Web.UI.Page
{
    iClass c = new iClass();
    public string customerId, orderCount, shippingCharges, prescriptionStr, paymentslip, mreq, ordId, deviceType, pickupReqStatus, apiResponse, gobpName;

    public string[] ordData = new string[30]; //14
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (!c.IsRecordExist("Select OrdAssignID From OrdersAssign Where FK_OrderID=" + Request.QueryString["id"] + " AND Fk_FranchID=" + Session["adminFranchisee"]))
            {
            ...
            int ordStatusTemp = Convert.ToInt32(c.GetReqData("OrdersAssign", "OrdAssignStatus", "FK_OrderID=" + Request.QueryString["id"] + " AND Fk_FranchID=" + Session["adminFranchisee"]));
            int ordReAssignTemp = Convert.ToInt32(c.GetReqData("OrdersAssign", "OrdReAssign", "FK_OrderID=" + Request.QueryString["id"] + "AND Fk_FranchID=" + Session["adminFranchisee"]));
        ...
        try
        {
            using (DataTable dtOrder = c.GetDataTable("Select * From OrdersData Where OrderID =" + ordIdx))
            {
                if (dtOrder.Rows.Count > 0)
                {
                    ...
                    ordData[23] = bRow["CourierOptions"] != DBNull.Value && bRow["CourierOptions"] != null && bRow["CourierOptions"].ToString() != "" ? bRow["CourierOptions"].ToString() : "-";
                    ...
                    if (bRow["OrderPayMode"] != DBNull.Value && bRow["OrderPayMode"] != null && bRow["OrderPayMode"].ToString() != "")
                    {
                        ...
                        else
                        {
                            ...
                            switch (Convert.ToInt32(bRow["OrderPayMode"]))
                            {
                                ...
                                case 1:
                                    ordData[12] = "Cash On Delivery";
                                    //14-Aug-23 change by Vinayak
                                    if (bRow["OrderPayStatus"] != DBNull.Value && bRow["OrderPayStatus"] != null)
                                    {
                                        ordData[13] = bRow["OrderPayStatus"].ToString() == "0" ? "UnPaid" : "Paid";
                                    }
                                    else
                    ...
                    using (DataTable dtCust = c.GetDataTable("Select CustomerName, CustomerMobile, CustomerEmail, CustomerAddress, CustomerCity, CustomerState, CustomerPincode From CustomersData Where CustomrtID = " + customerId))
                    {
                        if (dtCust.Rows.Count > 0)
                        {
                            ...
                            int ordAssignStatus = Convert.ToInt32(c.GetReqData("OrdersAssign", "OrdAssignStatus", "FK_OrderID=" + Request.QueryString["id"] + " AND Fk_FranchID=" + Session["adminFranchisee"]));
                    ...
                    //Payment Slip
                    if (c.IsRecordExist("Select OrderID From OrdersData Where OrderID=" + ordIdx))
                    {
                        StringBuilder strMarkup = new StringBuilder();
                        ...
                        using (DataTable dtRx = c.GetDataTable("Select OrdPayId, OrdPaySlip From OrdersPaymentSlip Where FK_OrderId = " + ordIdx))
                        {
                            if (dtRx.Rows.Count > 0)
                            {
                                ...
                                foreach (DataRow prow in dtRx.Rows)
                                {
                                    ...
                                    strMarkup.Append("<div class=\"imgContainer\">");
                                    //strMarkup.Append("<a href=\"" + rPath + "upload/prescriptions/" + prow["PrescriptionName"] + "\" data-fancybox=\"rxGroup\"><img src=\"" + rPath + "upload/prescriptions/" + prow["PrescriptionName"] + "\" class=\"width100\" /></a>");
                                    if (prow["OrdPaySlip"].ToString().Contains("http"))
                                    {
                                        strMarkup.Append("<a href=\"" + prow["OrdPaySlip"].ToString() + "\" data-fancybox=\"rxGroup\"><img src=\"" + prow["OrdPaySlip"].ToString() + "\" class=\"width100\" /></a>");
                                    }
                                    else
                                    {                                        
                                        strMarkup.Append("<a href=\"" + rPath + "upload/paymentslip/" + prow["OrdPaySlip"] + "\" data-fancybox=\"rxGroup\"><img src=\"" + rPath + "upload/paymentslip/" + prow["OrdPaySlip"] + "\" class=\"width100\" /></a>");
                                    }
                                    strMarkup.Append("</div>");
                                    strMarkup.Append("</div>");
                                    strMarkup.Append("</div>");
                    ...
                    {
                        txtDeliveryDate.Text = Convert.ToDateTime(bRow["EstimatedDeliveryDate"]).ToString("dd/MM/yyyy");
                    }
                    if (bRow["DispatchDate"] != DBNull.Value && bRow["DispatchDate"] != null && bRow["DispatchDate"].ToString() != "")
                    {
                        txtDate.Text = Convert.ToDateTime(bRow["DispatchDate"]).ToString("dd/MM/yyyy");
                    }

                    if (bRow["OrderSalesBillNumber"] != DBNull.Value && bRow["OrderSalesBillNumber"] != null && bRow["OrderSalesBillNumber"].ToString() != "")
                    {
                        txtSaleBill.Text = bRow["OrderSalesBillNumber"].ToString();
                    }
    ...
    protected void btnAccept_Click (object sender, EventArgs e)
    {
        try
        {
            ...
            // set OrderAssignStatus=5 bcoz order is in inprocess mode while it is accepted
            c.ExecuteQuery("Update OrdersAssign Set OrdAssignStatus=5 Where FK_OrderID=" + Request.QueryString["id"] + " AND Fk_FranchID=" + Session["adminFranchisee"]);
            // after insert its tracking history in OrderTracking table
            int trackMaxId = c.NextId("OrderTracking", "OrdTracID");
            int ordAssignId = Convert.ToInt32(c.GetReqData("OrdersAssign", "OrdAssignID", "FK_OrderID=" + Request.QueryString["id"] + " AND OrdAssignStatus=5 AND Fk_FranchID=" + Session["adminFranchisee"]));

            //***OrderTracking***
            // OrdTeacType 1 > Confirmed, 2 > Inprocess, 3 > Shipped, 4 > Delivered
            //insert entry for confirmed
            c.ExecuteQuery("Insert Into OrderTracking (OrdTracID, OrdTracDate, FK_OrderID, FK_OrdAssignID, OrdTracType, OrdTracMessage) " +
                ...
                ", 1, 'We have received your order, as soon as have packed your package, we will send it off for delivery.')");
            //insert entry for inprocess
            int trackMaxIdInc = trackMaxId + 1;
            c.ExecuteQuery("Insert Into OrderTracking (OrdTracID, OrdTracDate, FK_OrderID, FK_OrdAssignID, OrdTracType, OrdTracMessage) " +
                " Values (" + trackMaxIdInc + ", '" + DateTime.Now + "', " + Request.QueryString["id"] + ", " + ordAssignId +
                ", 2, 'We are getting your order ready to be shipped.  We will notify you when it has been sent.')");
            ...
            c.ExecuteQuery("Update OrdersData Set OrderStatus=5 Where OrderID=" + Request.QueryString["id"]);
            ...
            ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('success', 'Order accepted successfully');", true);
    ...
    protected void btnDispatch_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            DateTime dispatchDate = Convert.ToDateTime(arrDate[1] + "/" + arrDate[0] + "/" + arrDate[2]);

            string[] arrdeliveryDate = txtDeliveryDate.Text.Split('/');
            if (c.IsDate(arrdeliveryDate[1] + "/" + arrdeliveryDate[0] + "/" + arrdeliveryDate[2]) == false)
            {
                ScriptManager.RegisterClientScriptBlock(this, GetType(), "myScript", "TostTrigger('warning', 'Enter Valid Date');", true);
            }
            DateTime estimatedDate = Convert.ToDateTime(arrdeliveryDate[1] + "/" + arrdeliveryDate[0] + "/" + arrdeliveryDate[2]);
            ...
            c.ExecuteQuery("Update OrdersData Set OrderShipName='" + txtShipmentName.Text + "', OrderTrackingNo='" + txtTrackingNo.Text +
                "', DispatchDate='" + dispatchDate + "', EstimatedDeliveryDate='" + estimatedDate + "', OrderStatus=6, OrderSalesBillNumber = '" + txtSaleBill.Text + "' Where OrderID=" + Request.QueryString["id"]);

            // set OrderAssignStatus=6 as dispatched
            c.ExecuteQuery("Update OrdersAssign Set OrdAssignStatus=6 Where OrdReAssign=0 AND Fk_FranchID=" + Session["adminFranchisee"] + " AND FK_OrderID=" + Request.QueryString["id"]);
            ...
            if (!c.IsRecordExist("Select OrdTracID From OrderTracking Where FK_OrderID=" + Request.QueryString["id"] + " AND OrdTracType=3"))
            {
                ...
                int trackMaxId = c.NextId("OrderTracking", "OrdTracID");
                int ordAssignId = Convert.ToInt32(c.GetReqData("OrdersAssign", "OrdAssignID", "FK_OrderID=" + Request.QueryString["id"] + " AND OrdAssignStatus=6 AND Fk_FranchID=" + Session["adminFranchisee"]));
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        ...
        Response.Write("window.open('generate-mailing.aspx?id=" + Request.QueryString["id"]+"','_blank')");
        Response.Write("</script>");
        //Response.Redirect("generate-mailing.aspx?id=" + Request.QueryString["id"], true);
    }
        ...
        protected void chkDelivered_CheckedChanged(object sender, EventArgs e)
    {
        try
        {
            if (chkDelivered.Checked == true)
            {
                ...
                c.ExecuteQuery("Update OrdersData Set OrderStatus=7, OrderPayStatus=1, EstimatedDeliveryDate='" + DateTime.Now + "',FollowupLastDate='" + DateTime.Now + 
                ...
                c.ExecuteQuery("Update OrdersAssign Set OrdAssignStatus=7 Where FK_OrderID=" + Request.QueryString["id"] + " AND Fk_FranchID=" + Session["adminFranchisee"]);
                ...
                {
                    int trackMaxId = c.NextId("OrderTracking", "OrdTracID");
                    int ordAssignId = Convert.ToInt32(c.GetReqData("OrdersAssign", "OrdAssignID", "FK_OrderID=" + Request.QueryString["id"] + " AND OrdAssignStatus=7 AND Fk_FranchID=" + Session["adminFranchisee"]));
    ...
    protected void btnBack_Click(object sender, EventArgs e)
    {
        if (Request.QueryString["type"] != null)
        {
            ...
            {
                Response.Redirect("orders-report.aspx?type=" + Request.QueryString["type"].ToString(), false);
            }
            switch (Request.QueryString["type"])
            {
                case "new":
                    Response.Redirect("orders-report.aspx?type=new", false);
                    break;

                case "accepted":
                    Response.Redirect("orders-report.aspx?type=accepted", false);
                    break;

                case "rejected":
                    Response.Redirect("orders-report.aspx?type=rejected", false);
                    break;

                case "monthly":
                    Response.Redirect("orders-report.aspx?type=monthly", false);
                    break;

                case "dispatched":
                    Response.Redirect("orders-report.aspx?type=dispatched", false);
                    break;

                case "delivered":
                    Response.Redirect("orders-report.aspx?type=delivered", false);
                    break;

                case "rx":
                    Response.Redirect("orders-report.aspx?type=rx", false);
                    break;
                case "returned":
                    Response.Redirect("orders-report.aspx?type=returned", false);
                    break;
            }
        }
        else
        {
            Response.Redirect("orders-report.aspx", false);
        }
    }

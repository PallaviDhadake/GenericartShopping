<%@ Page Title="Survey Followup Report" Language="C#" MasterPageFile="~/franchisee/MasterFranchisee.master" AutoEventWireup="true" CodeFile="survey-followup-report.aspx.cs" Inherits="franchisee_survey_followup_report" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
	<div class="proNavPanel">
		...
		<ul class="proNav">
			<li><a href="monthly-order-followup.aspx">Monthly Orders Followup</a></li>
			<li><a class="act">Survey Followup</a></li>
		</ul>
	</div>
	<span class="space20"></span>

	<h2 class="pgTitle">Survey Followup Report</h2>
	<span class="space15"></span>

	<span class="medium bold_weight text-indigo mr-3">Total Surveys Done : <%= totalSurveys %></span>
	<span class="medium bold_weight text-indigo mr-3">Total Converted : <%= convertCount %></span>
	<span class="medium bold_weight text-indigo">Total Not Converted : <%= nonConvertCount %></span>
	<span class="space20"></span>

	<div id="viewOrder" runat="server">
		<div>
			<asp:GridView ID="gvSurvey" runat="server" CssClass="table table-striped table-bordered table-hover table-responsive-sm" GridLines="None"
				AutoGenerateColumns="false" OnRowDataBound="gvSurvey_RowDataBound" Width="100%">
				<RowStyle CssClass="" />
				...
				<Columns>
					...
					<asp:BoundField DataField="totalMonthlySurvey" HeaderText="Total Monthly Survey">
						<ItemStyle Width="5%" />
					</asp:BoundField>
					<asp:BoundField DataField="reOrdDate" HeaderText="Re-Order Date">
						<ItemStyle Width="5%" />
					</asp:BoundField>
					...
					<asp:BoundField DataField="surveyId">
						<HeaderStyle CssClass="HideCol" />
						<ItemStyle CssClass="HideCol" />
					</asp:BoundField>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-header">
					<h4 class="modal-title">Survey Follow Up</h4>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
				<%--<label class="container">Wrongly select as monthly order
					<input type="radio" checked="checked" name="radio" onclick="javascript: followupOption(1);">
					<span class="checkmark"></span>
				</label>
				<label class="container">Not interested for monthly order
					<input type="radio" name="radio" onclick="javascript: followupOption(2);">
					<span class="checkmark"></span>
				</label>
				<label class="container">Order Done, Call back in next month
					<input type="radio" name="radio" onclick="javascript: followupOption(3);">
					<span class="checkmark"></span>
				</label>
				<label class="container">Phone not picked 
	...
	<script>
		$('#modal-lg').on('show.bs.modal', function (event) {
			...
			var modal = $(this)
			//modal.find('.modal-title').text('Follow Up Survey: #' + recipient)
	...
	<script type = "text/javascript">
		...
		function SubmitFollowUp(orderRefX, optValX) {
			...
			$.ajax({
				type: "POST",
				url: "survey-followup-report.aspx/SurveyFollowUp",
